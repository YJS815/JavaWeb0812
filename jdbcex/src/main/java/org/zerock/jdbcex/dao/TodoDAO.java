...
public class TodoDAO {
    public String getTime() {
        String now = null;
        // try with resource 문 : Connection,PreparedStatement,ResultSet은 close()를 실행해야하기 때문에 사용
        try (Connection connection = ConnectionUtil.INSTANCE.getConnection();
             // PreparedStatement : SQL 실행하기 위한 사전 준비
    ...
    public String getTime2() throws Exception {
        // @Cleanup : lombok에서 지원하는 어노테이션으로 close를 실행해주는 기능 (lombok이 없으면 쓸수 없는 코드)
        @Cleanup Connection connection = ConnectionUtil.INSTANCE.getConnection();
        @Cleanup PreparedStatement pstmt = connection.prepareStatement("SELECT now()");
        @Cleanup ResultSet rs = pstmt.executeQuery();
        rs.next(); // 첫번째 데이터로 이동
        String now = rs.getString(1); // 첫번째 데이터를 취득
        return now;
    }
